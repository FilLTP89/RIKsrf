...
!SOURCE PARAMETERS:
    ...
    REAL,ALLOCATABLE,DIMENSION(:):: SRl,SRw,SRelem,SRmu,SRslip,SRmoment,SRstressdrop,SR,STF, ELIF_SRmoment
    INTEGER NSR,NT
!others
...
!Evaluating slip rates
    ...
    allocate(sr(NT),stf(NT),ELIF_SRmoment(NT))
    open(201,FILE='sr.dat')
    open(261,FILE='ELIF_MomentRate.dat')
...
!$OMP end parallel do
      ...
      ELIF_SRmoment = sr(:)*SRelem(i)*SRmu(i)

      do j=1,NT
       write(201,*) dt*(j-1), sr(j)
       write(261,*) dt*(j-1), ELIF_SRmoment(j)
      enddo
